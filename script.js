...
async function startAnalysis() {
  ...
  const workTypeLabel = workType || '미선택 (사진 기반 자동 판단)';

  const prompt = `당신은 건설현장 안전점검 전문가이며, 신입 감독관을 교육하는 베테랑 안전관리자입니다.
...
[작성 규칙]
1. 체크리스트 항목 비중:
   - 공종 특화 전문 항목: 약 50% (해당 공종에서만 발생하는 위험요소, 시공 품질 관련, 장비 점검 등)
   - 사진에서 관찰된 현장 상황 기반 항목: 약 30%
   - 일반 안전 항목(보호구, 정리정돈, 안전표지 등): 약 20% (가능한 통합하여 1~2개로)

2. 공종 특화 항목 작성 기준:
   - 해당 공종에서 사고가 자주 발생하는 핵심 위험 포인트를 우선 포함
   - 신입 감독관이 현장에서 흔히 놓치는 전문적인 점검사항 포함
   - 참고 법령에 직접 명시되지 않더라도 해당 공종의 실무에서 반드시 확인해야 하는 사항이면 포함 (이 경우 "현장 실무 기준"으로 표기)

3. 일반 안전 항목 통합:
   - 개인보호구(안전모, 안전화, 안전조끼 등)는 하나의 항목으로 통합
   - 작업장 정리정돈, 통로 확보 등도 하나로 통합
   - 세부 보호구별로 나누지 않을 것

4. 근거 표기: 각 항목에 법령 조항 또는 "현장 실무 기준" 표기
5. 10~15개 항목 생성, 각 항목에 위험도("위험", "주의", "양호" 중 택1) 판단
