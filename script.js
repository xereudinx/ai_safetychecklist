...
async function downloadPDF() {
  ...
  (checklistData.groups || []).forEach(group => {
    ...
    (group.items || []).forEach(item => {
      ...
      html += `<span style="display:inline-block;font-size:8px;font-weight:600;padding:1px 6px;border-radius:3px;background:${bg};color:${color};margin-top:2px;">${item.level}</span>`;
      html += `</div>`;
    });
    html += `</div>`;
  });
