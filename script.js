...
async function startAnalysis() {
  ...
  const checked = Array.from(document.querySelectorAll('#workTypeGrid input:checked')).map(cb => cb.value);
...
1단계: 사용자가 선택한 공종을 최우선으로 반영하여 해당 공종의 전문 점검항목을 먼저 작성
2단계: 그 다음 현장 사진을 분석하여 사진에서 추가로 확인되는 위험요소나 점검항목을 보충
3단계: 마지막으로 기본 안전 점검 항목을 통합하여 추가

[작성 규칙]
1. 체크리스트 개수: 최소 10개, 최대 15개 (반드시 준수)

2. 구성 순서와 비율:
   ① 선택 공종 기반 전문 항목을 먼저 작성 (전체의 50~60%)
     - 사용자가 선택한 공종의 핵심 점검사항을 최우선 배치
     - 초보 공사감독이 놓치기 쉬운 전문 항목 위주
   ② 사진 분석 기반 추가 항목 (전체의 20~30%)
     - 사진에서 관찰된 현장 상황 중 선택 공종에 포함되지 않은 추가 위험요소
   ③ 기본 안전 점검 항목 (전체의 10~20%)
     - 보호구, 안전표지, 정리정돈 등은 통합하여 1~2개로

3. groups 배치 순서도 위 순서를 따를 것:
   - 첫 번째 group: 선택 공종 항목
   - 두 번째 group: 사진 분석 추가 항목
   - 세 번째 group: 기본 안전 점검
...
5. 기본 안전 항목은 세부적으로 나누지 말고 통합 작성
