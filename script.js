...
// === Fixed mandatory items per work type ===
// These are guaranteed to appear in the checklist whenever the matching
// work type checkbox is selected, regardless of what the AI returns.
// keywords: if an AI-generated item already contains ALL of these keywords,
// it's treated as a duplicate and the fixed item is not added again.
const FIXED_ITEMS_BY_WORKTYPE = {
  '굴착기 작업': [
    { text: '굴착기 운전자의 적정 자격 확인 (조종면허 또는 교육이수증 보유 여부)', keywords: ['굴착기', '자격'] },
    { text: '굴착기 운전자 안전띠 착용 여부', keywords: ['굴착기', '안전띠'] }
  ]
};

// Merge fixed items into the AI-generated checklist data.
// Only adds items for work types the user actually selected, and only
// if an equivalent item isn't already present in the AI's output.
function applyFixedItems(data, checkedWorkTypes) {
  if (!data || !Array.isArray(data.groups)) return data;

  checkedWorkTypes.forEach(workType => {
    const fixedItems = FIXED_ITEMS_BY_WORKTYPE[workType];
    if (!fixedItems) return;

    const allTexts = data.groups.flatMap(g => (g.items || []).map(i => i.text || ''));

    const missingItems = fixedItems.filter(fi =>
      !allTexts.some(t => fi.keywords.every(k => t.includes(k)))
    );

    if (missingItems.length > 0) {
      data.groups.push({
        title: `[필수] ${workType} 안전 확인 항목`,
        items: missingItems.map(fi => ({ text: fi.text, level: '주의' }))
      });
    }
  });
...
[공종별 필수 포함 항목 - 해당 공종 선택 시 반드시 포함]
- 굴착기 작업:
  ① 전조등, 후방영상장치, 후사경 작동여부 및 설치상태 점검
  ② 굴착기 법정 필수 검사 실시 여부
  ③ 굴착기 운전자의 적정 자격 확인 (조종면허 또는 교육이수증 보유 여부)
  ④ 굴착기 운전자 안전띠 착용 여부
- 콘크리트 펌프카 작업:
  ① 붐대와 고압선 접촉 여부 확인
  ② 사면과 이격거리 2m 이상 확보 여부
- 자재 인양 작업:
  ① 인양자재 요동방지를 위한 유도로프 사용 여부
  ② 자재 줄걸이 방법의 정확성 및 단단한 결속 여부
...
6. 법령 근거 표기 시 약어(A, B, C, D)를 사용하지 말고 법령 원문 이름을 그대로 표기할 것
   예) (A.163) → (산업안전보건기준에 관한 규칙 제163조)
   예) (B.1) → (건설기계 안전기준에 관한 규칙 제1조)
   예) (C.2) → (안전보건관리규정 제2조)
   예) (D.5) → (건설공사안전관리지침 제5조)
   법령 근거가 없더라도 현장에서 중요한 항목은 반드시 포함
...
[출력 형식 - 반드시 JSON만 출력, 다른 텍스트 없이]
{
  ...
  try {
    ...
    checklistData = applyFixedItems(checklistData, checked);
...
async function downloadPDF() {
  ...
  // Title
  let html = `<h1 style="font-size:17px;text-align:center;margin:0 0 14px;font-weight:800;">현장 안전점검 체크리스트</h1>`;
  ...
  (checklistData.groups || []).forEach(group => {
    ...
    (group.items || []).forEach(item => {
      html += `<div style="padding:4px 0;font-size:10px;line-height:1.5;border-bottom:1px solid #f5f5f5;">`;
