...
[요청]
1. 모든 체크리스트 항목에는 반드시 관련 법령·지침의 근거 조항을 포함할 것.
2. 사진과 선택한 공종을 기반으로, 신입사원이 흔히 놓치는 위험 요소와 공종 특화 위험 요소를 포함.
3. 공종이 선택된 경우, 공종 관련 전문 항목이 체크리스트의 약 50%를 차지하도록 비중 조절.
4. 개인보호구(안전모·안전화·안전조끼), 추락방지, 작업장 정리정돈 등 일반 안전 항목은 세부적으로 나누지 말고 하나의 항목으로 통합.
5. 법적 근거에 명시되지 않은 전문 공종 포인트라도 실제 현장에서 중요한 경우 반드시 포함.
6. 결과는 신입사원이 현장에서 바로 체크할 수 있는 수준으로 구체적이고 실무적이어야 함.
7. 10~15개 항목 생성, 각 항목에 위험도("위험", "주의", "양호" 중 택1) 판단.
